...
#include "test_LPF_ModuleList.h"
#include "test_LPF_HeapSize.h"
#include <math.h>
...
uint32_t g_nElapsedMilliSeconds = 0;
uint32_t nPrevCycles = 0;

uint32_t AWE_Packet_Buffer[MAX_COMMAND_BUFFER_LEN];
uint32_t AWE_Reply_Buffer[MAX_COMMAND_BUFFER_LEN];
...
uint32_t g_MasterHeap[MASTER_HEAP_SIZE];
uint32_t g_FastbHeap[FASTB_HEAP_SIZE];
uint32_t g_SlowHeap[SLOW_HEAP_SIZE];
...
uint32_t g_module_descriptor_table_size = sizeof(g_module_descriptor_table) / sizeof(g_module_descriptor_table[0]);

/* USER CODE BEGIN PV */
int32_t I2S_RxBuffer_L[SAMPLE_BUFFER_SIZE];
int32_t I2S_RxBuffer_R[SAMPLE_BUFFER_SIZE];
int32_t AWE_OutBuffer[SAMPLE_BUFFER_SIZE];
uint32_t I2S_RxBuffer_L_Index = 0;
uint32_t I2S_RxBuffer_R_Index = 0;
/* USER CODE END PV */

/* Private function prototypes -----------------------------------------------*/
void AWEInstanceInit(void);
...
int main(void)
{
  ...
  /* Import samples to core. */
  awe_audioImportSamples(&g_AWEInstance, I2S_RxBuffer_R, 1, 1, Sample32bit);
  awe_audioExportSamples(&g_AWEInstance, AWE_OutBuffer, 1, 1, Sample32bit);
...
void AWEInstanceInit(void)
{
    ...
    awe_initPin(&s_InputPin, 1, NULL);
    awe_initPin(&s_OutputPin, 1, NULL);
